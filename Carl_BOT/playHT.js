...
function postToSocialMedia(classList) {
    ...
    if (classList.contains("twitter")) {
        const url = `https://twitter.com/_lijojoseph`;
        window.open(url);
    } else if (classList.contains("linkedin")) {
        const url = `https://www.linkedin.com/in/lijojoseph0021/`;
        window.open(url);
    } else if (classList.contains("whatsapp")) {
        window.open(`https://www.instagram.com/_lj21?igsh=MW53a3ViNndmYTZ1aA==`);
    }
}
